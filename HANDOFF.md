...
| Base map (chart) | Esri `Canvas/World_Dark_Gray_Base` + `World_Dark_Gray_Reference` (ArcGIS Online, no API key) | Two standard tile layers combined in one `L.layerGroup` (base fill/roads + a transparent labels overlay), so the single "Chart (dark)" entry in the layer control adds/removes both together. Was CartoDB's `dark_all` tiles until 2026-09-10 — CARTO closed off anonymous access to `basemaps.cartocdn.com`; a tile request there now returns a 200 OK containing a watermarked "API KEY REQUIRED" placeholder image instead of map data. Same failure shape as the NOAA dataset relocations elsewhere in this table: a "successful" response that isn't actually the data it looks like. |
